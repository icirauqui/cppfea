...
# imports for solving with sparse matrix
...
# print matrix k without premature line breaks
...
def main():
	...
	# Apply custom forces in the nodes at the end of the 'truss'
	force_nodes = [17, 18, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310]
	force_nodes = [5, 10, 15]
	force_nodes = [6, 12, 18, 24]
	for i in range(len(force_nodes)):
		force_nodes[i] = force_nodes[i] - 1
	print(force_nodes, 'force nodes')
	
	for fn in force_nodes:
		f[2*fn+0] += 1.0
		f[2*fn+1] += 1.0
	print(len(force_nodes), 'custom forces applied')
	print(sum(f), 'sum of forces')
